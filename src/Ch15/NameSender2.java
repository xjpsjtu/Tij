package Ch15;
//:NameSender2.java
//An applet that sends an email address via a CGI GET, using Java 1.02
import java.awt.*;
import java.applet.*;
import java.net.*;
import java.io.*;
public class NameSender2 extends Applet{
	...
	public boolean action(Event evt, Object arg){
		if(evt.target.equals(send)){
			...
			if(end.indexOf('.') == -1){
				l.setText("Protion after '@' must " + "have an extension, such as '.com'");
				l2.setText("");
				return true;
			}
			//Build an encode the email data:
			String emailData = "name=" + URLEncoder.encode(name.getText().trim()) + "&email=" + URLEncoder.encode(email.getText().trim().toLowerCase()) + "&submit=Submit";
