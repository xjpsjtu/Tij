package Ch15;
//:NameSender.java
//An applet that sends an email address as a datagram, using Java 1.02
import java.awt.*;
import java.applet.*;
import java.net.*;
import java.io.*;
public class NameSender extends Applet implements Runnable{
	...
	public boolean action(Event evt, Object arg){
		if(evt.target.equals(send)){
			if(p1 != null){
				//p1.stop(); Deprecated in Java 1.2
				Thread remove = p1;
				p1 = null;
				remove.interrupt();
			}
			...
			//Everything is OK, so send the name. Get a fresh buffer, so it's zeroed. For some reason
			//you must use a fixed size rather than calculating the size dynamically:
			...
			p1 = new Thread(this);
			p1.start();
			l2.setText("Waitting for verification " + ++vcount);
		}
		else return super.action(evt, arg);
		return true;
	}
	//The thread portion of the applet watches for the reply to come back from server:
	public void run(){
		try{
			s.receive(dp);
		}catch(Exception e){
